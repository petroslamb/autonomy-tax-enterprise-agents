...
The evidence converges from three independent contexts:
...
**Collaborative software development.** The Xu et al. study cited above examined the Copilot effect across open-source projects. The productivity gains accrued to peripheral (less-experienced) contributors, while the maintenance and quality-control burden shifted to the expert pool. The pattern held even when controlling for project characteristics and contributor experience levels.
...
In each case, the failures were primarily in the control layer — spending limits, input sanitization, privilege boundaries, state verification — not in the model's reasoning. The systems surrounding the AI weren't engineered for autonomous operation.
...
Consider a scenario playing out at companies right now: a team deploys an agent pipeline for internal document processing. It works. Three weeks later, during an audit preparation, someone asks a simple question — *why did the agent classify this contract as low-risk?* Nobody can answer. The agent's reasoning isn't logged, the intermediate steps aren't traced, and the decision tree that led to the classification exists only as a vanished sequence of token probabilities. The answer to "why?" is, architecturally, "we don't know." That isn't an edge case — it's a common pattern in early agent deployments, and the survey data below suggests it's widespread.

*[Note: This scenario is a composite illustrating patterns documented in Shapira et al. and in the Akto survey findings below. Organizations that discover they can't explain agent decisions after deployment are a common pattern in practitioner reports.]*

Deloitte's survey found that 74% of organizations intend to deploy agentic AI at scale, but only 21% have a mature AI governance model in place [[Deloitte 2026](https://www.deloitte.com/content/dam/assets-zone3/us/en/docs/services/consulting/2026/state-of-ai-2026.pdf)]. Separately, a security-focused survey found that only 21% of enterprises have full visibility into the actions their AI agents take [[Akto 2025](https://www.prnewswire.com/news-releases/aktos-2025-state-of-agentic-ai-security-report-finds-only-21-of-enterprises-have-visibility-302635105.html); note: survey weighted toward security-conscious organizations]. These are different surveys measuring different constructs in different populations — but the convergence on the same number is striking: in both cases, roughly four in five organizations lack the governance maturity or operational visibility that autonomous agents require.

If you can't explain why your agent did what it did, you don't have a governance problem. You have a liability.

The Governance Tax has three components:
...
The stakes of this decision are concrete: choose too much autonomy and you absorb all three taxes at full force. Choose too little and you leave the throughput gains on the table. The scorecard below is designed to make that choice explicit rather than intuitive.
...
**The Level 2 → 2.5 distinction matters for tax exposure.** In a Level 2 pipeline, each node makes a single LLM call — the blast radius is small and the output is easy to review. In a Level 2.5 pipeline, each node runs a multi-turn ReAct loop with tool access (filesystem, APIs, shell commands), producing richer outputs but with a larger action surface per step. The control comes from two architectural constraints: first, the pipeline sequence itself is fixed and deterministic — no node chooses which node runs next. Second, each node reads a defined structured input and produces a defined structured output (a JSON payload, a markdown document, a typed schema). Only that artifact crosses the node boundary. Human review gates sit at predetermined decision points rather than on every output. The result is a system where seniors review at gates rather than on every action, the blast radius of any single node failure is bounded by its output contract, and the audit trail is a sequence of typed artifacts rather than opaque chat logs.

The architecture decision should be driven by tax exposure. The thresholds below are order-of-magnitude guidance, not empirically derived cutoffs — use them as starting points and adjust for your domain:

| Condition | Level 2.5 (pipeline + gates) | Level 3 (autonomous) |
|---|---|---|
| Error cost per incident | Roughly > $100 or reputational | Roughly < $10 and recoverable |
...
The economic logic: when the cost of reviewing every output exceeds the expected cost of unreviewed errors, skipping review is rational.
...
*All inputs are illustrative — Klarna has not published error rates, remediation costs, or per-review costs. The purpose is to show how the logic shifts across assumptions, not to claim any threshold as universal.*
...
But notice what makes this work: low individual error cost, high reversibility, minimal regulatory exposure. Most enterprise processes that leaders *want* to automate with agents — procurement, compliance review, legal analysis, financial approvals — violate at least two of those conditions. A procurement agent that approves one wrong $5,000 invoice wipes out the savings from 1,000 correct ones. The Klarna case is instructive precisely because it is the exception, not the rule.
...
**What would disprove this:** Any of the following would constitute substantive counter-evidence: (1) an independent post-mortem dataset showing model accuracy as the primary root cause in >50% of enterprise agent failures; (2) a peer-reviewed study demonstrating that improved model capability *reduces* rather than shifts control-layer costs; or (3) widespread enterprise adoption of Level 3+ agents in regulated domains without corresponding investment in observability and governance. If these emerge by December 2027, this thesis is wrong.
...
*[Publication note: Internal repository links (casebook, source manifest, method notes) should be replaced with public URLs or hosted assets before external publication.]*

**§1 Hook:** Proxy post-mortem (SRC-026).
